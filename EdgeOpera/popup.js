...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function requestCoverMany(hosts) {
    return new Promise(resolve => {
      api.runtime.sendMessage({ action: "coverInfoMany", hosts }, (res) => {
        if (api.runtime.lastError) return resolve({});
        resolve((res && res.covers) || {});
      });
    });
  }
  ...
  function refreshDomainsPanel() {
    ...
    fetchTabDomains().then(domains => {
      ...
      requestCoverMany(hosts).then(covers => {
        domainCovers = covers;
        renderDomainsList(domains, covers);
      });
    });
  }
  ...
  function persistProxies(list, done) {
  ...
  function renderProxies() {
    ...
    currentProxies.forEach(p => {
      ...
      inp.addEventListener("change", () => {
        ...
        persistProxies(currentProxies.map(item => Object.assign({}, item, { enabled: item.id === p.id })), () => {
  ...
  function loadState() {
    api.storage.local.get(["proxyConfig", "proxyServers", "proxyRules", "directRules", "proxyLists", "lastProxyError", "extensionEnabled"], (res) => {
      ...
      if (JSON.stringify(migrated) !== JSON.stringify(prev) || res.extensionEnabled !== extensionEnabled) persistProxies(migrated, afterMigrate);
      else {
  ...
  els.saveProxy.addEventListener("click", () => {
    ...
    if (editingProxyId != null) {
      ...
      persistProxies(currentProxies.concat(item), () => {
  ...
  els.deleteProxy.addEventListener("click", () => {
    if (editingProxyId == null) return;
    persistProxies(currentProxies.filter(p => p.id !== editingProxyId), () => {
  ...
  els.saveDomains.addEventListener("click", () => {
    ...
    const finish = () => {
      closeDomainsPanel();
      if (added || removed || changed) reloadActiveTab();
    };
  ...
  els.powerBtn.addEventListener("click", () => {
    ...
    api.storage.local.set({ extensionEnabled }, () => refreshPowerBtn());
  });
